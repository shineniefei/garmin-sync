...
SYNC_CONFIG = {
    'SOURCE_GARMIN_AUTH_DOMAIN': 'cn',
    'SOURCE_GARMIN_EMAIL': 'shineniefei@qq.com',
    'SOURCE_GARMIN_PASSWORD': '',
    'SYNC_GARMIN_AUTH_DOMAIN': 'com',
    'SYNC_GARMIN_EMAIL': 'shineniefei@qq.com',
...
async def upload_activity(main_client, sync_client, activity_id):
    try:
        ...
        download_folder = os.path.join(
            FIT_DIR,
            'FIT-' + main_client.email + "-" + main_client.auth_domain)
        if not os.path.exists(download_folder):
            os.mkdir(download_folder)

        unzip_folder = os.path.join(
            FIT_DIR,
            "FIT-UNZIP-" + main_client.email + "-" + main_client.auth_domain)
        # 如果下载类型是fit需要新增解压路径
        ...
        unzip_fit_name_list = await unzip_fit(
            str(activity_id) + '.zip', unzip_folder, download_folder)
        for file_name in unzip_fit_name_list:
            ...
            flag = await sync_client.upload_activity(file_path,
                                                     upload_file_type,
                                                     activity_id)
        # 选择WEBDAV
        if LOCAL_OR_WEBDAV and flag:
            activity_fit_zip_name = JIAN_GOU_YUN_WEBDAV_PATH + '/' + JIAN_GOU_YUN_WEBDAV_FIT_FOLDER + '/' + str(
                ma.activityId) + '.zip'
...
if __name__ == "__main__":
    ...
    # 建立主Garmin 链接
    main_client = GarminConnect(SYNC_CONFIG["SOURCE_GARMIN_EMAIL"],
                                SYNC_CONFIG["SOURCE_GARMIN_PASSWORD"],
                                SYNC_CONFIG["SOURCE_GARMIN_AUTH_DOMAIN"],
                                False)
    # 建立Sync Garmin 链接
    sync_client = GarminConnect(SYNC_CONFIG["SYNC_GARMIN_EMAIL"],
                                SYNC_CONFIG["SYNC_GARMIN_PASSWORD"],
                                SYNC_CONFIG["SYNC_GARMIN_AUTH_DOMAIN"], False)

    garmin_cookie = GarminCookie(SYNC_CONFIG["SOURCE_GARMIN_EMAIL"],
                                 SYNC_CONFIG["SOURCE_GARMIN_AUTH_DOMAIN"],
                                 SYNC_CONFIG["SYNC_GARMIN_EMAIL"],
                                 SYNC_CONFIG["SYNC_GARMIN_AUTH_DOMAIN"])

    loop = asyncio.get_event_loop()

    # 加载Cookie
    load_garmin_db_tasks = [
        asyncio.ensure_future(load_garmin_db(main_client, True,
                                             garmin_cookie)),
        asyncio.ensure_future(load_garmin_db(sync_client, False,
                                             garmin_cookie))
    ]
    ...
    # 如果配置了WEBDAV
    if LOCAL_OR_WEBDAV:
        for i in jianguoyun_client.client.ls(JIAN_GOU_YUN_WEBDAV_PATH + '/' +
                                             JIAN_GOU_YUN_WEBDAV_FIT_FOLDER):
    ...
    for id in upload_activityID_list:
        upload_activity_task = asyncio.ensure_future(
            upload_activity(main_client, sync_client, id))
