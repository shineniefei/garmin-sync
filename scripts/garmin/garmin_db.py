...
class GarminDB:
    ...
    def saveCookeie(self, is_main=True):
        ...
        encrypt_main_email = self.aesChiper.encrypt(
            self.garmin_cookie.main_email)
        encrypt_main_auth_domain = self.aesChiper.encrypt(
            self.garmin_cookie.main_auth_domain)
        encrypt_sync_email = self.aesChiper.encrypt(
            self.garmin_cookie.sync_email)
        encrypt_sync_auth_domain = self.aesChiper.encrypt(
            self.garmin_cookie.sync_auth_domain)
        ...
        with SqliteDB(os.path.join(DB_DIR, self.garmin_db_path)) as db:
            ...
            exists_query_set = db.execute(
                exists_select_sql,
                (encrypt_main_email, encrypt_main_auth_domain,
                 encrypt_sync_email, encrypt_sync_auth_domain)).fetchall()

            query_size = len(exists_query_set)
            if query_size == 0:
                db.execute(init_sql,
                           (encrypt_main_email, encrypt_main_auth_domain,
                            encrypt_sync_email, encrypt_sync_auth_domain))
                id = exists_query_set = db.execute(
                    exists_select_sql,
                    (encrypt_main_email, encrypt_main_auth_domain,
                     encrypt_sync_email,
                     encrypt_sync_auth_domain)).fetchall()[0][0]

                db.execute(cookie_insert_sql, (serial_cookeie, str(id)))
            else:
                db.execute(cookie_insert_sql,
                           (serial_cookeie, exists_query_set[0][0]))
    ...
    def uploaded_activity(self, activity_id):
        with SqliteDB(self.garmin_db_path) as db:
            db.execute(
                'insert into uploaded_activity (garmin_cookie_id, activity_id) values (?, ?)',
                (self.garmin_cookie.id, activity_id))
