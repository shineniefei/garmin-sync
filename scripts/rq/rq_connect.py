...
class RQConnect:
    ...
    def __init__(self, email, password, rqdbpath):
        self.headers = {
            "Host":
            "www.runningquotient.cn",
            "Content-Type":
            "application/x-www-form-urlencoded",
            "authorization":
            "PtCNFNv2FrKVmWuKzALA",
            "accept-encoding":
            "br;q=1.0, gzip;q=0.9, deflate;q=0.8",
            "user-agent":
            "RQ Run/2.9.4 (cn.runningquotient.RQ-Run; build:12; iOS 14.6.0) Alamofire/2.9.4",
            "accept-language":
            "zh-Hans-HK;q=1.0, en-HK;q=0.9, en-GB;q=0.8, yue-Hant-HK;q=0.7",
        }
    ...
    async def login(self, aesChiper):
        ...
        if result['syscode'] == 200:
            ...
            with SqliteDB(self.rqdbpath) as db:
                ...
                # 加密access_token
                encrypt_access_token = aesChiper.encrypt(
                    result['data']['access_token'])
                # 加密user_id
                encrypt_user_id = aesChiper.encrypt(
                    str(result['data']['user_id']))

                # 如果为0说明是第一次执行脚步，则插入数据
                if query_size == 0:
                    db.execute(
                        'insert into user_info (email,user_id,access_token) values (?, ?, ?)',
                    ...
                    if query_size == 1:
                        ...
                        for row in query_set:
                            db.execute('delete from user_info where id = ? ',
                                       (row[0], ))
                        db.execute(
                            'insert into user_info (email,user_id,access_token) values (?, ?, ?)',
                            (encrypt_email, encrypt_user_id,
    ...
    # 判断Token是否过期
    async def isExpiredToken(self, aesChiper, encrypt_user_id,
                             encrypt_access_token):
        ...
        getUserInfoHeader['x-user-token'] = aesChiper.decrypt(
            encrypt_access_token)
        ...
        response = await self.req.get(getUserInfoUrl,
