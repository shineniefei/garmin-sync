...
class JianGuoYunClient:
    ...
    def __init__(self) -> None:
        try:
            ...
            self.client = Client(base_url=WEBDAV_CONFIG['WEBDAV_URL'],
                                 auth=(WEBDAV_CONFIG['WEBDAV_USERNAME'],
            ...
            # 判断是否存在存放FIT解压文件文件夹
            if not self.client.exists(JIAN_GOU_YUN_WEBDAV_PATH + '/' +
                                      JIAN_GOU_YUN_WEBDAV_FIT_UNZIP_FOLDER):
                self.client.mkdir(JIAN_GOU_YUN_WEBDAV_PATH + '/' +
                                  JIAN_GOU_YUN_WEBDAV_FIT_UNZIP_FOLDER)

        except Exception as err:
            print(err)
            raise JianGuoYunInitError(
                "初始化坚果云异常，请检查帐号密码是否正确或网络链接情况！！！\n异常信息为：" + str(err)) from err
